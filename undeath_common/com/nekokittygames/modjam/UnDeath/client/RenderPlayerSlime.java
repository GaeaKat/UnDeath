...
public class RenderPlayerSlime extends RenderLiving {
    ...
    private static final ResourceLocation field_110897_a = new ResourceLocation(
                                                                 "undeath",
                                                                 "textures/entity/playerSlime.png");
    private ModelBase                     scaleAmount;
    private RenderItem                    itemRenderer;
    private Random                        rand;
    private static float[][]              posShifts      = {
            { -0.75F, 1.5F, 0.5F }, { 0F, 1.5F, 0.5F }, { 0.75F, 1.5F, 0.5F },
            { -0.75F, 1.5F, 0F }, { 0F, 1.5F, 0F }, { 0.75F, 1.5F, 0F },
            { -0.75F, 1.5F, -0.5F }, { 0F, 1.5F, -0.5F },
            { 0.75F, 1.5F, -0.5F },
            ...
            { -0.75F, 0.25F, 0.5F }, { 0F, 0.25F, 0.5F },
            { 0.75F, 0.25F, 0.5F }, { -0.75F, 0.25F, 0F }, { 0F, 0.25F, 0F },
            { 0.75F, 0.25F, 0F }, { -0.75F, 0.25F, -0.5F },
            { 0F, 0.25F, -0.5F }, { 0.75F, 0.25F, -0.5F }, };

    public RenderPlayerSlime(ModelBase par1ModelBase, ModelBase par2ModelBase,
            float par3) {
    ...
    public void doRender(Entity par1Entity, double par2, double par4,
        ...
        final EntityPlayerSlime pSlime = (EntityPlayerSlime) par1Entity;
        rand.setSeed(1337L);
        float shiftX;
        float shiftY;
        float shiftZ;
        int shift = 0;
        final float blockScale = 0.75f;// *(1f/pSlime.getSlimeSize());
        final float timeDelta = (float) (360.0 * (System.currentTimeMillis() & 0x3FFFL) / 0x3FFFL);
        GL11.glPushMatrix();
        GL11.glDisable(GL11.GL_LIGHTING);
        GL11.glTranslatef((float) par2, (float) par4, (float) par6);
        final EntityItem custItem = new EntityItem(pSlime.worldObj);
        custItem.hoverStart = 0f;
        for (final ItemStack item : pSlime.items) {
    ...
    protected void scaleSlime(EntityPlayerSlime par1EntitySlime, float par2) {
        ...
        final float f2 = (par1EntitySlime.field_70812_c + (par1EntitySlime.field_70811_b - par1EntitySlime.field_70812_c)
                * par2)
                / (f1 * 0.5F + 1.0F);
    ...
    protected int shouldRenderPass(EntityLivingBase par1EntityLivingBase,
            int par2, float par3) {
        return this.shouldSlimeRenderPass(
                (EntityPlayerSlime) par1EntityLivingBase, par2, par3);
    }

    /**
     * Determines whether Slime Render should pass or not.
     */
    protected int shouldSlimeRenderPass(EntityPlayerSlime par1EntitySlime,
            int par2, float par3) {
